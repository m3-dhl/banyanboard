...
globs: ["src/services/*.ts", "src/controllers/*.ts", "src/config/*.ts", "app.ts", "*.tsx", "frontend/src/**", "memory-bank/tasks/*.md"]
...
- For permanent-delete or irreversible-action features, explicitly enumerate out-of-scope items (soft delete, undo, recovery UI) in the spec's Scope Boundaries — treat this as a required element, not optional, to prevent scope creep across implementation phases.
...
| Negative-scope enumeration required for destructive-action features (prevents scope creep) | [reflection-TASK-012.md](../reflection/reflection-TASK-012.md) | 2026-06-17 |
